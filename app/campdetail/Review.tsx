...
export default function Review({ contentId }: ReviewProps) {
  ...
  return (
    <div>
      ...
      />
      <div className="mt-6">
        <h3 className="text-xl font-bold mb-4">작성된 리뷰</h3>
        {reviews.map((review, index) => (
          <div key={index} className="bg-gray-50 p-6 rounded-lg shadow mb-4">
            <div className="flex justify-between">
              <p className="text-orange-400">{review.userId}</p>
              <div>
            ...
            <p className="pt-8">{review.review}</p>
          </div>
        ))}
      </div>
    </div>
